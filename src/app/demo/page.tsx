...
type Model = {
  ...
  category: string;
  src: string | null;
  image: string; // Unsplash hero image
  size: string;
  description: string;
  features: string[];
  creator: string;
    ...
    models: [
      {
        ...
        sourceLabel: "Ver escena 3D original ↗",
        tech: "Splat",
      },
    ],
  },
    ...
    models: [
      {
        ...
        image: unsplash("photo-1600510424051-30d592a75353"),
        size: "16 MB",
        description:
    ...
    models: [
      {
        ...
        size: "31 MB",
        description:
          "El trono de Napoleón Bonaparte capturado como Gaussian Splat — telas, dorados, tallas y detalles imperiales reconstruidos fotograméticamente. Vista3D permite exhibir piezas únicas de forma segura a una audiencia global.",
        features: [
    ...
    models: [
      {
        ...
        size: "58 MB",
        description:
          "Interior real del Restaurant Hueb en Suiza capturado como Gaussian Splat — mesas, decoración, iluminación y ambiente reconstruidos fotograméticamente. El cliente recorre el restaurante antes de reservar.",
        features: [
...
export default function DemoPage() {
  ...
  // Reset to photo view whenever the industry tab changes
  useEffect(() => {
    setShow3D(false);
  }, [industryId]);
          ...
          {show3D ? (
            ...
            <>
              ...
              <button
                ...
                className="absolute top-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 px-4 py-2 rounded-full bg-black/70 backdrop-blur-md border border-white/20 text-xs font-medium text-white/80 hover:text-white hover:bg-black/90 transition"
              >
                ← Volver a la foto
              </button>
              {/* Controls hint */}
            ...
            <>
              ...
              {/* CTA: Ver en 3D — centered at bottom */}
              {model.src && (
                <div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-2">
                  <button
                    onClick={() => setShow3D(true)}
                    className="flex items-center gap-2.5 px-6 py-3 rounded-full bg-white text-black font-semibold text-sm shadow-2xl hover:bg-white/90 active:scale-95 transition-all"
                  >
                    <span className="text-base leading-none">✦</span>
                    Ver en 3D Gaussian Splat
                    <span className="text-xs opacity-60">→</span>
                  </button>
                  <p className="text-[10px] text-white/50 font-mono">
                    Interactivo · {model.size} · WebGL2
                  </p>
                </div>
              )}
            </>
